...
if [ -n "${ZEN_VERSION:-}" ]; then
	TAG="$ZEN_VERSION"
	[ "${VERBOSE:-0}" = "1" ] && echo "[init] TAG from ZEN_VERSION=$TAG" >&2
elif [ -x "$BIN" ]; then
	_ver=$("$BIN" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -n1)
	if [ -n "${_ver:-}" ]; then case "$_ver" in v*) TAG="$_ver" ;; *) TAG="v$_ver" ;; esac; [ "${VERBOSE:-0}" = "1" ] && echo "[init] TAG from BIN $BIN version=$_ver -> $TAG" >&2; else TAG=$(curl -fsSL --connect-timeout 5 --max-time 10 "https://api.github.com/repos/${REPO}/releases/latest" 2>/dev/null | grep -o '"tag_name"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4); [ -z "${TAG:-}" ] && { echo "failed to fetch latest version and parse local BIN version" >&2; exit 1; }; echo "[init] BIN version parse failed, fetched latest TAG=$TAG" >&2; fi
else
	...
	[ -z "${TAG:-}" ] && { echo "failed to fetch latest version (network error)" >&2; exit 1; }
...
DUMP="${DUMP:-0}"
VERBOSE="${VERBOSE:-0}"
USER_AGENT="${USER_AGENT:-opencode/1.18.24 ai-sdk/provider-utils/4.0.23 runtime/bun/1.3.14}"
...
check_new_version() {
    local latest
    [ "${VERBOSE:-0}" = "1" ] && echo "[check] current TAG=$TAG, fetching latest ..." >&2
    ...
    if [ -z "$latest" ]; then [ "${VERBOSE:-0}" = "1" ] && echo "[check] fetch latest failed (empty)" >&2; return 0; fi
    [ "${VERBOSE:-0}" = "1" ] && echo "[check] latest=$latest current=$TAG" >&2
    if [ "$latest" != "$TAG" ]; then
        _cur="${TAG#v}"; _lat="${latest#v}"
        if [ "$(printf '%s\n%s\n' "$_cur" "$_lat" | sort -V | head -n1)" = "$_lat" ] && [ "$_cur" != "$_lat" ]; then
            [ "${VERBOSE:-0}" = "1" ] && echo "[check] latest $latest < current $TAG, skip downgrade" >&2
            return 0
        fi
        ...
        if [ "${ZEN_AUTO_UPDATE:-0}" = "1" ]; then
            ...
            TAG="$latest"
            TAG_UPDATED=1
            [ "${VERBOSE:-0}" = "1" ] && echo "[check] TAG_UPDATED=1 (auto)" >&2
        elif [ -t 0 ] && [ "${1:-}" != "--quiet" ]; then
            printf 'Update to %s? [y/N]: ' "$latest" >&2
            read -r ans || ans=""
            [ "${VERBOSE:-0}" = "1" ] && echo "[check] input ans=$ans" >&2
            case "$ans" in y|Y|yes|YES) TAG="$latest"; TAG_UPDATED=1; [ "${VERBOSE:-0}" = "1" ] && echo "[check] TAG_UPDATED=1 (Y)" >&2 ;; *) echo "Keeping $TAG" >&2 ;; esac
        else
            echo "Auto-updating to $latest ..." >&2
            TAG="$latest"
            TAG_UPDATED=1
            [ "${VERBOSE:-0}" = "1" ] && echo "[check] TAG_UPDATED=1 (non-interactive auto)" >&2
        fi
    else
        [ "${VERBOSE:-0}" = "1" ] && echo "[check] already latest" >&2
    fi
}
...
ensure_binary() {
    ...
    if [ ! -x "$BIN" ]; then
        need_dl=1
        [ "${VERBOSE:-0}" = "1" ] && echo "[ensure] BIN not executable, need_dl=1" >&2
    elif [ -n "${TAG_UPDATED:-}" ]; then
        need_dl=1
        echo "Version changed to $TAG, re-downloading ..." >&2
    else
        [ "${VERBOSE:-0}" = "1" ] && echo "[ensure] BIN exists TAG=$TAG TAG_UPDATED=${TAG_UPDATED:-0} skip download" >&2
    fi
    if [ "$need_dl" = "1" ]; then
