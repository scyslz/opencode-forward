...
func newEgressManager(prefer string, probeURL4, probeURL6 string, interval time.Duration, resolver *net.Resolver) *egressManager {
	transports := map[string]*http.Transport{
		"4": {DialContext: makeDialContext("4", resolver), ForceAttemptHTTP2: true, MaxIdleConns: 100, MaxIdleConnsPerHost: 100, IdleConnTimeout: 90 * time.Second, TLSHandshakeTimeout: 10 * time.Second, ResponseHeaderTimeout: upstreamTimeout},
		"6": {DialContext: makeDialContext("6", resolver), ForceAttemptHTTP2: true, MaxIdleConns: 100, MaxIdleConnsPerHost: 100, IdleConnTimeout: 90 * time.Second, TLSHandshakeTimeout: 10 * time.Second, ResponseHeaderTimeout: upstreamTimeout},
	}
	if probeURL4 == "" {
